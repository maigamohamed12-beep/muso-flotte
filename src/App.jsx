...
// ─── CONSTANTES ──────────────────────────────────────────────
const E_NEXT  = { 'Planifié': 'En cours', 'En cours': 'Effectué' }
const E_STEPS = ['Planifié', 'En cours', 'Effectué']
const fmt     = n => (n || 0).toLocaleString('fr-FR')
const daysDiff = d => Math.ceil((new Date(d) - new Date()) / 86400000)

function exportHTML(title, headers, rows, fname) {
  const th = headers.map(h => `<th style="background:#00536A;color:#fff;padding:8px 12px">${h}</th>`).join('')
  const tr = rows.map((r, i) => `<tr style="background:${i % 2 ? '#f0f4f6' : '#fff'}">${r.map(c => `<td style="padding:7px 12px;border-bottom:1px solid #e5e7eb">${c ?? ''}</td>`).join('')}</tr>`).join('')
  const a = document.createElement('a')
  a.href = URL.createObjectURL(new Blob([`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title></head><body style="font-family:Arial;padding:20px"><h2 style="color:#00536A">ONG MUSO — ${title}</h2><table style="width:100%;border-collapse:collapse;margin-top:14px"><thead><tr>${th}</tr></thead><tbody>${tr}</tbody></table></body></html>`], { type: 'text/html' }))
...
.toast{position:fixed;top:18px;right:22px;padding:11px 18px;border-radius:11px;color:white;font-weight:700;font-size:13px;z-index:200;box-shadow:0 4px 20px rgba(0,0,0,.2);}
...
@media(max-width:768px){
  .sidebar{transform:translateX(-100%);}.main{margin-left:0!important;}.ham{display:flex!important;}.mnav{display:block;}
...
// ─── UI COMPONENTS ────────────────────────────────────────────
function SBadge({ s }) {
  const m = { 'Disponible':'b-dispo','En mission':'b-mission','En entretien':'b-entretien','Hors service':'b-hs','En attente':'b-attente','Validée':'b-validee','Terminée':'b-terminee','Rejetée':'b-rejetee','Planifié':'b-planifie','En cours':'b-encours','Effectué':'b-effectue' }
  ...
  return (
    <div>
      <div className="stepper">
        {E_STEPS.map((s, i) => <div key={s} className={`step ${i < idx ? 'step-done' : i === idx ? 'step-cur' : ''}`} />)}
      </div>
      <div style={{ display:'flex', justifyContent:'space-between' }}>
        {E_STEPS.map((s, i) => <span key={s} style={{ fontSize:9, fontWeight:700, textAlign:'center', flex:1, color: i < idx ? '#059669' : i === idx ? '#FF8725' : '#d1d5db' }}>{s}</span>)}
      </div>
    </div>
  )
}
...
function MissionCard({ m, roleProfil, avail, onValider, onRejeter, onCloturer }) {
  ...
  return (
    <div className={`csm ${m.combinee ? 'ccomb' : ''}`}>
      ...
      {roleProfil === 'Supply Chain' && m.statut === 'En attente' && (
      ...
      {roleProfil === 'Supply Chain' && m.statut === 'Validée' && (
...
function LoginScreen() {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#00536A 0%,#00919E 60%,#003d50 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
        <div style={{ background:'white', borderRadius:22, padding:36, width:'100%', maxWidth:400, boxShadow:'0 24px 80px rgba(0,0,0,.22)' }}>
          <div style={{ textAlign:'center', marginBottom:28 }}>
            <div style={{ width:70, height:70, background:'linear-gradient(135deg,#00536A,#00919E)', borderRadius:20, display:'flex', alignItems:'center', justifyContent:'center', fontSize:36, margin:'0 auto 14px' }}>🚘</div>
          ...
          <div className="fg">
            ...
            <input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} placeholder="••••••••" className="fi" />
          </div>
          ...
          <button onClick={handleLogin} disabled={loading}
            style={{ width:'100%', padding:'13px', fontSize:15, fontWeight:700, borderRadius:11, border:'none', cursor:'pointer', color:'white', background:'linear-gradient(135deg,#00536A,#00919E)', opacity: loading ? 0.7 : 1, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
            {loading ? <><span className="spin" style={{ width:16, height:16, borderWidth:2 }} /> Connexion…</> : 'Se connecter →'}
          </button>
          <div style={{ marginTop:16, fontSize:12, color:'#9ca3af', textAlign:'center' }}>Mot de passe oublié ? Contactez votre administrateur.</div>
        </div>
      </div>
    </>
  )
...
export default function App() {
  ...
  // ── Auth — directement dans App sans hook externe ──
  ...
  useEffect(() => {
    ...
    fetchAll()
    // Temps réel
    const ch1 = supabase.channel('v').on('postgres_changes', {event:'*', schema:'public', table:'vehicules'}, fetchVehicules).subscribe()
    const ch2 = supabase.channel('m').on('postgres_changes', {event:'*', schema:'public', table:'missions'}, fetchMissions).subscribe()
    const ch3 = supabase.channel('e').on('postgres_changes', {event:'*', schema:'public', table:'entretiens'}, fetchEntretiens).subscribe()
    const ch4 = supabase.channel('c').on('postgres_changes', {event:'*', schema:'public', table:'carburant'}, fetchCarburant).subscribe()
    const ch5 = supabase.channel(`n_${user.id}`).on('postgres_changes', {event:'INSERT', schema:'public', table:'notifications', filter:`destinataire_id=eq.${user.id}`}, payload => setNotifs(p => [payload.new, ...p])).subscribe()
  ...
  async function fetchAll() {
    await Promise.all([fetchVehicules(), fetchMissions(), fetchEntretiens(), fetchCarburant(), fetchPaysSites(), fetchNotifs()])
  }

  async function fetchVehicules() {
    let q = supabase.from('vehicules').select('*, chauffeur:chauffeur_id(id, nom, initiales)')
    if (fP) q = q.eq('pays', fP)
    if (fS) q = q.eq('site', fS)
    const { data } = await q.order('immat')
    if (data) setVehicles(data)
  }

  async function fetchMissions() {
    let q = supabase.from('missions').select('*, vehicule:vehicule_id(id,immat,marque,modele), chauffeur:chauffeur_id(id,nom), demandeurs:mission_demandeurs(demandeur:demandeur_id(id,nom))')
    if (fP) q = q.eq('pays', fP)
    if (fS) q = q.eq('site', fS)
  ...
  async function fetchEntretiens() {
    let q = supabase.from('entretiens').select('*, vehicule:vehicule_id(id,immat,marque,modele)')
    if (fP) q = q.eq('pays', fP)
    if (fS) q = q.eq('site', fS)
  ...
  async function fetchCarburant() {
    let q = supabase.from('carburant').select('*, vehicule:vehicule_id(id,immat,marque)')
    if (fP) q = q.eq('pays', fP)
    if (fS) q = q.eq('site', fS)
    const { data } = await q.order('date', { ascending: false })
    if (data) setCarburant(data)
  }

  async function fetchPaysSites() {
    const { data } = await supabase.from('pays_sites').select('*').order('pays').order('site')
    if (data) {
  ...
  async function fetchNotifs() {
    const { data } = await supabase.from('notifications').select('*').eq('destinataire_id', user.id).order('created_at', { ascending: false }).limit(50)
    if (data) setNotifs(data)
  }

  const psNoms = Object.fromEntries(Object.entries(ps).map(([p, sites]) => [p, sites.map(s => s.site)]))
  const avail = vehicles.filter(v => v.statut === 'Disponible')
  const pending = missions.filter(m => m.statut === 'En attente')
  const unread = notifs.filter(n => !n.lu).length
  const roleProfil = profil?.profil || ''
  ...
  const allSites = fP ? (psNoms[fP] || []) : [...new Set(vehicles.map(v => v.site))]
  ...
  const dispo = useMemo(() => {
    ...
    return { byP, g: { dispo:vehicles.filter(v=>v.statut==='Disponible').length, mission:vehicles.filter(v=>v.statut==='En mission').length, entretien:vehicles.filter(v=>v.statut==='En entretien').length, hs:vehicles.filter(v=>v.statut==='Hors service').length, total:vehicles.length }}
  }, [vehicles])
  ...
  const mPer = useMemo(() => {
    ...
    fMis.forEach(m => {
      ...
      r[mo].tot++
      if (m.statut === 'Validée') r[mo].v++
      else if (m.statut === 'Terminée') r[mo].t++
      else if (m.statut === 'En attente') r[mo].a++
      else if (m.statut === 'Rejetée') r[mo].r2++
    })
    return r
  }, [fMis])
  ...
  async function envoyerNotif(destinataireId, message) {
    await supabase.from('notifications').insert({ destinataire_id: destinataireId, message })
  }

  const handleValider = async (m, vehiculeId) => {
    const veh = vehicles.find(v => v.id === vehiculeId)
    await save(async () => {
      await supabase.from('missions').update({ statut:'Validée', vehicule_id:vehiculeId, chauffeur_id:veh?.chauffeur_id }).eq('id', m.id)
      await supabase.from('vehicules').update({ statut:'En mission' }).eq('id', vehiculeId)
      const dIds = m.demandeurs?.map(d => d.demandeur?.id).filter(Boolean) || []
      for (const uid of dIds) await envoyerNotif(uid, `✅ Mission "${m.destination}" validée — ${veh?.immat}`)
  ...
  const handleRejeter = async (m) => {
    await save(async () => {
      await supabase.from('missions').update({ statut:'Rejetée' }).eq('id', m.id)
      const dIds = m.demandeurs?.map(d => d.demandeur?.id).filter(Boolean) || []
      for (const uid of dIds) await envoyerNotif(uid, `❌ Mission "${m.destination}" rejetée.`)
  ...
  const handleCloturer = async (m, kmRetour) => {
    await save(async () => {
      await supabase.from('missions').update({ statut:'Terminée', km_retour:parseInt(kmRetour) }).eq('id', m.id)
      await supabase.from('vehicules').update({ statut:'Disponible', km:parseInt(kmRetour) }).eq('id', m.vehicule?.id)
      notify('Mission clôturée !')
    })
  }

  const handleAvancer = async (e) => {
    const ns = E_NEXT[e.statut]
    if (!ns) return
    await save(async () => {
      await supabase.from('entretiens').update({ statut:ns }).eq('id', e.id)
      if (ns === 'Effectué') await supabase.from('vehicules').update({ statut:'Disponible' }).eq('id', e.vehicule?.id)
      notify(ns === 'Effectué' ? 'Entretien effectué — véhicule Disponible !' : 'Statut mis à jour.')
    })
    setConfirm(null)
  }

  const handleSupprimer = async (v) => {
    const actif = missions.some(m => m.vehicule_id === v.id && m.statut === 'Validée')
    if (actif) { notify('Véhicule en mission active.', 'error'); setConfirm(null); return }
    await save(async () => {
      await supabase.from('vehicules').delete().eq('id', v.id)
      notify(`${v.immat} supprimé.`)
    })
    setConfirm(null)
  }
  ...
  const handleCombiner = async () => {
    ...
    await save(async () => {
      const { data: nm } = await supabase.from('missions').insert({
        ...
        pays: tm[0].pays, site: tm[0].site, statut: 'En attente', combinee: true
      }).select().single()
      if (nm) {
        await supabase.from('mission_demandeurs').insert(dIds.map(uid => ({ mission_id: nm.id, demandeur_id: uid })))
        await supabase.from('missions').delete().in('id', selComb)
        for (const uid of dIds) await envoyerNotif(uid, `🔗 Mission combinée : "${nm.destination}"`)
      }
  ...
  const ALL_TABS = roleProfil === 'Admin'
    ? [{ k:'backend', l:'Backend', icon:'🖥️' }, ...SC_TABS]
    : roleProfil === 'Supply Chain' ? SC_TABS
    : [{ k:'missions', l: roleProfil === 'Chauffeur' ? 'Mes missions' : 'Mes demandes', icon: roleProfil === 'Chauffeur' ? '🚗' : '📋' }]

  const showMissions = roleProfil === 'Chauffeur'
  ...
  const Chips = () => (
    <div className="chips">
      {Object.keys(psNoms).map(p => <button key={p} className={`chip ${fP === p ? 'active' : ''}`} onClick={() => { setFP(fP === p ? '' : p); setFS('') }}>🌍 {p}</button>)}
      {fP && allSites.map(s => <button key={s} className={`chip ${fS === s ? 'active' : ''}`} onClick={() => setFS(fS === s ? '' : s)}>📍 {s}</button>)}
      {allMois.map(m => <button key={m} className={`chip ${fM === m ? 'active' : ''}`} onClick={() => setFM(fM === m ? '' : m)}>📅 {m}</button>)}
      {(fP || fS || fM) && <button className="chip chip-r" onClick={() => { setFP(''); setFS(''); setFM('') }}>✕ Reset</button>}
    </div>
  )

  // ── Écrans de chargement et login ──
  if (!authReady) return (
    <>
      <style>{CSS}</style>
  ...
  return (
    <>
      ...
      <div className="shell">
        ...
        <div className="main">
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ position:'relative' }}>
                <button className="btn btn-ghost btn-sm" onClick={() => { setBell(b => !b); toutMarquerLu() }}>
                  🔔 {unread > 0 && <span style={{ background:'#FF8725', color:'white', borderRadius:20, padding:'1px 6px', fontSize:10, fontWeight:700 }}>{unread}</span>}
                </button>
                {bell && (
                  <div className="notif-panel">
                    ...
                    {notifs.length === 0
                      ...
                      : notifs.map(n => <div key={n.id} className={`ni ${n.lu ? '' : 'ni-u'}`}><div style={{ marginBottom:2 }}>{n.message}</div><div style={{ fontSize:10, color:'#9ca3af' }}>{new Date(n.created_at).toLocaleString('fr-FR')}</div></div>)
                    }
                  </div>
                )}
              </div>
              <div className="user-pill">
                ...
                <div>
                  <div style={{ fontSize:12, fontWeight:700, color:'#00536A', lineHeight:1.2 }}>{profil?.nom || user.email}</div>
                  <div style={{ fontSize:10, color:'#9ca3af' }}>{roleProfil}</div>
                </div>
              </div>
          ...
          <div className="page">
            ...
            {tab === 'dashboard' && (
              <div>
                ...
                <div className="grid4" style={{ marginBottom:14 }}>
                  <KPICard icon="🚗" label="Flotte"    value={`${vehicles.length} véh.`} sub={`${dispo.g.dispo} disponibles`} bg="#e6f2f5" />
                  <KPICard icon="📍" label="Km total"  value={fmt(vehicles.reduce((s,v) => s+v.km, 0))} sub="km" bg="#e6f5f1" ic="#059669" />
                ...
                {alerts.length > 0 && (
                  <div onClick={() => setTab('alertes')} style={{ background:'#fff3e8', border:'1px solid #fdba74', borderRadius:12, padding:'12px 16px', marginBottom:14, cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div><div style={{ fontWeight:700, color:'#FF8725' }}>⚠️ {alerts.length} alerte(s)</div><div style={{ fontSize:12, color:'#9a6900', marginTop:2 }}>{alerts.filter(a=>a.type==='danger').length} critique(s) · {alerts.filter(a=>a.type==='warning').length} avertissement(s)</div></div>
                ...
                <div className="grid2" style={{ marginBottom:14 }}>
                  <div className="card">
                    ...
                    <div className="stat-grid">
                      {[{l:'Disponibles',n:dispo.g.dispo,bg:'#d1fae5',c:'#065f46',icon:'✅'},{l:'En mission',n:dispo.g.mission,bg:'#e6f2f5',c:'#00536A',icon:'🚗'},{l:'Entretien',n:dispo.g.entretien,bg:'#fff3e8',c:'#d97706',icon:'🔧'},{l:'Hors service',n:dispo.g.hs,bg:'#fee2e2',c:'#dc2626',icon:'⛔'}].map(s => (
                ...
                <div className="card">
                  ...
                  {Object.entries(mPer).sort(([a],[b]) => b.localeCompare(a)).map(([mois, s]) => (
                    <div key={mois} style={{ marginBottom:12 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}><span style={{ fontSize:13, fontWeight:700 }}>{mois}</span><span style={{ fontSize:11, color:'#9ca3af' }}>{s.tot}</span></div>
                      <div className="pbar">
            ...
            {tab === 'alertes' && (
              <div>
                {alerts.length === 0 && <div className="card" style={{ textAlign:'center', color:'#059669', fontWeight:700, padding:32 }}>✅ Aucune alerte</div>}
                {alerts.map((a, i) => (
                  <div key={i} className={`alertc ${a.type === 'danger' ? 'adanger' : 'awarning'}`}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:12, fontWeight:700, marginTop:3, color: a.type==='danger' ? '#dc2626' : '#d97706' }}>{a.days <= 0 ? `⛔ Expiré depuis ${Math.abs(a.days)} j` : `⏳ J-${a.days} (${a.date})`}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {tab === 'vehicules' && (
              <div>
                <div className="ph">
                  <div className="pt">Véhicules ({vehicles.length})</div>
                  <div style={{ display:'flex', gap:8 }}>
                    <button className="btn btn-outline btn-sm" onClick={() => exportHTML('Parc', ['Immat.','Marque','Km','Statut','Pays','Site'], vehicles.map(v => [v.immat, `${v.marque} ${v.modele}`, fmt(v.km), v.statut, v.pays, v.site]), 'vehicules')}>📄 Export</button>
                ...
                <div className="tw">
                  <table>
                    ...
                    <tbody>
                      {vehicles.map(v => {
                        const aD = v.assurance ? daysDiff(v.assurance) : 999
                        const viD = v.visite ? daysDiff(v.visite) : 999
                        return (
                          <tr key={v.id}>
                            ...
                            <td>{v.pays}/{v.site}</td>
                            <td style={{ color:aD<=30?'#dc2626':aD<=60?'#d97706':'inherit' }}>{v.assurance||'—'}{aD<=60&&<div style={{ fontSize:10, fontWeight:700 }}>{aD<=0?'⛔ Expirée':`J-${aD}`}</div>}</td>
                            <td style={{ color:viD<=30?'#dc2626':viD<=60?'#d97706':'inherit' }}>{v.visite||'—'}{viD<=60&&<div style={{ fontSize:10, fontWeight:700 }}>{viD<=0?'⛔ Expirée':`J-${viD}`}</div>}</td>
                            <td><div style={{ display:'flex', gap:5 }}><button className="btn btn-danger btn-xs" onClick={() => setConfirm({ msg:`Supprimer ${v.immat} ?`, danger:true, onOk:() => handleSupprimer(v) })}>🗑</button></div></td>
                          </tr>
                        )
                      })}
                    </tbody>
            ...
            {tab === 'missions' && (
              <div>
                <div className="ph">
                  <div className="pt">{roleProfil==='Chauffeur'?'Mes missions':roleProfil==='Staff'?'Mes demandes':'Missions'}</div>
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                    {roleProfil === 'Supply Chain' && pending.length >= 2 && <button className="btn btn-a btn-sm" onClick={() => setCombineModal(true)}>🔗 Combiner</button>}
                    {roleProfil === 'Supply Chain' && <button className="btn btn-outline btn-sm" onClick={() => exportHTML('Missions', ['Destination','Demandeur','Véhicule','Départ','Statut'], fMis.map(m => [m.destination, m.demandeurs?.map(d=>d.demandeur?.nom).join(', '), m.vehicule?.immat||'—', m.depart, m.statut]), 'missions')}>📄 Export</button>}
                    {roleProfil !== 'Chauffeur' && <button className="btn btn-p btn-sm" onClick={() => setSheet('mission')}>+ Nouvelle</button>}
                  </div>
                </div>
                {roleProfil === 'Supply Chain' && <Chips />}
                <div className="grid2">
                  {showMissions.map(m => <MissionCard key={m.id} m={m} roleProfil={roleProfil} avail={avail} onValider={handleValider} onRejeter={handleRejeter} onCloturer={handleCloturer} />)}
                </div>
              </div>
            )}
            ...
            {tab === 'carburant' && (
              <div>
                ...
                <div className="tw">
                  <table>
                    <thead><tr><th>Véhicule</th><th>Date</th><th>Litres</th><th>Coût (FCFA)</th><th>Station</th><th>Km</th><th>Site</th></tr></thead>
                    <tbody>{fCarb.map(c => <tr key={c.id}><td><b>{c.vehicule?.immat}</b></td><td>{c.date}</td><td style={{ color:'#FF8725', fontWeight:700 }}>{c.litres}L</td><td>{fmt(c.cout)}</td><td>{c.station}</td><td>{fmt(c.km)}</td><td>{c.site}</td></tr>)}</tbody>
                  </table>
                </div>
              </div>
            )}
            ...
            {tab === 'entretien' && (
              <div>
                ...
                <div style={{ background:'#e6f2f5', border:'1px solid #b2d8df', borderRadius:10, padding:'10px 14px', marginBottom:14, fontSize:13, color:'#00536A' }}>
                  <b>Workflow :</b> Planifié → En cours → Effectué. À <b>Effectué</b>, le véhicule repasse <b>Disponible</b>.
                </div>
                <div className="tw">
                  <table>
                    ...
                    <tbody>
                      {fEnt.map(e => (
                        <tr key={e.id}>
                          ...
                          <td>{E_NEXT[e.statut] ? <button className="btn btn-p btn-xs" onClick={() => setConfirm({ msg:`Marquer "${E_NEXT[e.statut]}" ?${E_NEXT[e.statut]==='Effectué'?' Véhicule → Disponible.':''}`, onOk:() => handleAvancer(e) })}>→ {E_NEXT[e.statut]}</button> : <span style={{ fontSize:12, color:'#059669', fontWeight:700 }}>✅ Terminé</span>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* BACKEND */}
            {tab === 'backend' && (roleProfil === 'Admin') && (
              <div>
                <div className="ph"><div className="pt">🖥️ Vue Backend</div><span style={{ fontSize:12, background:'#fee2e2', color:'#dc2626', padding:'4px 10px', borderRadius:20, fontWeight:700 }}>🔒 Admin</span></div>
                ...
                {[
                  { label:'🚗 VEHICULES', data:vehicles, cols:['immat','marque','statut','km','pays','site'], r:v=>({immat:v.immat,marque:`${v.marque} ${v.modele}`,statut:v.statut,km:fmt(v.km),pays:v.pays,site:v.site}) },
                ...
                <div>
                  ...
                  <div className="tw" style={{ marginBottom: editPS ? 14 : 0 }}>
                    <table>
                      ...
                      <tbody>
                        {Object.entries(ps).flatMap(([pays, sites]) =>
                          sites.map((s, i) => (
                            <tr key={pays+s.site}>
                              ...
                              {editPS && <td><div style={{ display:'flex', gap:5 }}>
                                ...
                                <button className="btn btn-danger btn-xs" onClick={() => setConfirm({ msg:`Supprimer "${s.site}" ?`, danger:true, onOk: async () => { await supabase.from('pays_sites').delete().eq('id', s.id); fetchPaysSites(); setConfirm(null); notify('Site supprimé.') } })}>🗑</button>
                              </div></td>}
                            </tr>
                          ))
                        )}
                  ...
                  {editPS && (
                    <div className="grid2">
                      <div className="card" style={{ padding:14 }}>
                        ...
                        <button className="btn btn-p btn-sm" style={{ width:'100%', justifyContent:'center' }} onClick={async () => { await supabase.from('pays_sites').insert({ pays:newPays, site:'Capitale' }); setNewPays(''); fetchPaysSites(); notify('Pays ajouté.') }}>Ajouter</button>
                      </div>
                      <div className="card" style={{ padding:14 }}>
                        <div style={{ fontWeight:700, color:'#00536A', marginBottom:10, fontSize:13 }}>+ Ajouter un site</div>
                        <div className="fg"><label className="fl">Pays</label><select value={newSite.pays} onChange={e => setNewSite({...newSite, pays:e.target.value})} className="fi"><option value="">-- Choisir --</option>{Object.keys(ps).map(p => <option key={p}>{p}</option>)}</select></div>
                        ...
                        <button className="btn btn-p btn-sm" style={{ width:'100%', justifyContent:'center' }} onClick={async () => { await supabase.from('pays_sites').insert({ pays:newSite.pays, site:newSite.val }); setNewSite({pays:'',val:''}); fetchPaysSites(); notify('Site ajouté.') }}>Ajouter</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* MODALS */}
      {sheet === 'mission' && (
        <Modal title="Nouvelle demande de mission" onClose={() => setSheet(null)} onSubmit={() => save(async () => {
          const { data: m } = await supabase.from('missions').insert({ destination:mf.destination, depart:mf.depart, retour:mf.retour, objet:mf.objet, passagers:parseInt(mf.passagers)||1, pays:mf.pays, site:mf.site, statut:'En attente' }).select().single()
          if (m) await supabase.from('mission_demandeurs').insert({ mission_id:m.id, demandeur_id:user.id })
          setMf({ destination:'', depart:'', retour:'', objet:'', passagers:1, pays:'', site:'' })
          notify('Demande soumise !')
        })} submitLabel="Soumettre" loading={saving}>
          <div className="fgrid">
            <FI full label="Destination" value={mf.destination} onChange={v => setMf({...mf, destination:v})} />
            <FI label="Date départ" type="date" value={mf.depart} onChange={v => setMf({...mf, depart:v})} />
            <FI label="Date retour" type="date" value={mf.retour} onChange={v => setMf({...mf, retour:v})} />
            <FI full label="Objet" value={mf.objet} onChange={v => setMf({...mf, objet:v})} />
            <FI label="Passagers" type="number" value={mf.passagers} onChange={v => setMf({...mf, passagers:v})} />
            <FS label="Pays" value={mf.pays} onChange={v => setMf({...mf, pays:v, site:''})} options={Object.keys(psNoms)} />
            <FS full label="Site" value={mf.site} onChange={v => setMf({...mf, site:v})} options={mf.pays ? psNoms[mf.pays] : []} />
          </div>
        </Modal>
      )}

      {sheet === 'vehicule' && (
        <Modal title="Ajouter un véhicule" onClose={() => setSheet(null)} onSubmit={() => save(async () => {
          await supabase.from('vehicules').insert({ immat:vf.immat, marque:vf.marque, modele:vf.modele, annee:parseInt(vf.annee)||null, km:parseInt(vf.km)||0, pays:vf.pays, site:vf.site, assurance:vf.assurance||null, visite:vf.visite||null, statut:vf.statut })
          setVf({ immat:'', marque:'', modele:'', annee:'', km:'', pays:'', site:'', assurance:'', visite:'', statut:'Disponible' })
          notify('Véhicule ajouté !')
        })} loading={saving}>
          <div className="fgrid">
            <FI label="Immatriculation" value={vf.immat} onChange={v => setVf({...vf, immat:v})} />
            <FI label="Marque" value={vf.marque} onChange={v => setVf({...vf, marque:v})} />
            <FI label="Modèle" value={vf.modele} onChange={v => setVf({...vf, modele:v})} />
            <FI label="Année" type="number" value={vf.annee} onChange={v => setVf({...vf, annee:v})} />
            <FI label="Km actuel" type="number" value={vf.km} onChange={v => setVf({...vf, km:v})} />
            <FS label="Statut" value={vf.statut} onChange={v => setVf({...vf, statut:v})} options={['Disponible','En entretien','Hors service']} />
            <FS label="Pays" value={vf.pays} onChange={v => setVf({...vf, pays:v, site:''})} options={Object.keys(psNoms)} />
            <FS label="Site" value={vf.site} onChange={v => setVf({...vf, site:v})} options={vf.pays ? psNoms[vf.pays] : []} />
            <FI label="Fin assurance" type="date" value={vf.assurance} onChange={v => setVf({...vf, assurance:v})} />
            <FI label="Fin visite tech." type="date" value={vf.visite} onChange={v => setVf({...vf, visite:v})} />
          </div>
        </Modal>
      )}

      {sheet === 'entretien' && (
        <Modal title="Planifier un entretien" onClose={() => setSheet(null)} onSubmit={() => save(async () => {
          const veh = vehicles.find(v => v.id === ef.vehicule_id)
          await supabase.from('entretiens').insert({ vehicule_id:ef.vehicule_id, type:ef.type, date:ef.date, km:parseInt(ef.km)||0, cout:parseInt(ef.cout)||0, prestataire:ef.prestataire, notes:ef.notes, pays:veh?.pays||'', site:veh?.site||'' })
          await supabase.from('vehicules').update({ statut:'En entretien' }).eq('id', ef.vehicule_id)
          setEf({ vehicule_id:'', type:'', date:'', km:'', cout:'', prestataire:'', notes:'' })
          notify('Entretien planifié !')
        })} loading={saving}>
          <div className="fgrid">
            <FS full label="Véhicule" value={ef.vehicule_id} onChange={v => setEf({...ef, vehicule_id:v})} options={vehicles.map(v => ({v:v.id, l:`${v.immat} · ${v.marque} ${v.modele}`}))} />
            <FS label="Type" value={ef.type} onChange={v => setEf({...ef, type:v})} options={['Vidange','Révision générale','Changement pneus','Freins','Climatisation','Carrosserie','Réparation moteur','Autre']} />
            <FI label="Date prévue" type="date" value={ef.date} onChange={v => setEf({...ef, date:v})} />
            <FI label="Km compteur" type="number" value={ef.km} onChange={v => setEf({...ef, km:v})} />
            <FI label="Coût estimé FCFA" type="number" value={ef.cout} onChange={v => setEf({...ef, cout:v})} />
            <FI label="Prestataire" value={ef.prestataire} onChange={v => setEf({...ef, prestataire:v})} />
            <FI full label="Notes" value={ef.notes} onChange={v => setEf({...ef, notes:v})} placeholder="Ex: En attente de pièces…" />
          </div>
        </Modal>
      )}

      {sheet === 'carburant' && (
        <Modal title="Enregistrer un ravitaillement" onClose={() => setSheet(null)} onSubmit={() => save(async () => {
          const veh = vehicles.find(v => v.id === cf.vehicule_id)
          await supabase.from('carburant').insert({ vehicule_id:cf.vehicule_id, date:cf.date, litres:parseFloat(cf.litres)||0, cout:parseInt(cf.cout)||0, station:cf.station, km:parseInt(cf.km)||0, pays:veh?.pays||'', site:veh?.site||'' })
          setCf({ vehicule_id:'', date:'', litres:'', cout:'', station:'', km:'' })
          notify('Ravitaillement enregistré !')
        })} loading={saving}>
          <div className="fgrid">
            <FS full label="Véhicule" value={cf.vehicule_id} onChange={v => setCf({...cf, vehicule_id:v})} options={vehicles.map(v => ({v:v.id, l:`${v.immat} · ${v.marque}`}))} />
            <FI label="Date" type="date" value={cf.date} onChange={v => setCf({...cf, date:v})} />
            <FI label="Km compteur" type="number" value={cf.km} onChange={v => setCf({...cf, km:v})} />
            <FI label="Litres" type="number" value={cf.litres} onChange={v => setCf({...cf, litres:v})} />
            <FI label="Coût total FCFA" type="number" value={cf.cout} onChange={v => setCf({...cf, cout:v})} />
            <FI full label="Station" value={cf.station} onChange={v => setCf({...cf, station:v})} />
          </div>
        </Modal>
      )}

      {combineModal && (
        <Modal title="🔗 Combiner des missions" wide onClose={() => { setCombineModal(false); setSelComb([]); setCombObj('') }} onSubmit={handleCombiner} submitLabel="Combiner" loading={saving}>
          ...
          <div style={{ maxHeight:260, overflowY:'auto', marginBottom:14 }}>
            {pending.map(m => (
              <div key={m.id} onClick={() => setSelComb(s => s.includes(m.id) ? s.filter(x=>x!==m.id) : [...s,m.id])}
                ...
                <div style={{ width:18, height:18, borderRadius:4, border:`2px solid ${selComb.includes(m.id)?'#00919E':'#d1d5db'}`, background:selComb.includes(m.id)?'#00919E':'white', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginTop:2 }}>
                  {selComb.includes(m.id) && <span style={{ color:'white', fontSize:11, fontWeight:700 }}>✓</span>}
                </div>
                <div><div style={{ fontWeight:700, color:'#00536A' }}>📍 {m.destination}</div><div style={{ fontSize:12, color:'#6b7280' }}>{m.demandeurs?.map(d=>d.demandeur?.nom).join(', ')} · {m.objet}</div><div style={{ fontSize:11, color:'#9ca3af' }}>📅 {m.depart} → {m.retour} · 👥 {m.pass} pass.</div></div>
              </div>
            ))}
          </div>
          {selComb.length >= 2 && (
            <><div style={{ background:'#e6f2f5', borderRadius:10, padding:'10px 14px', marginBottom:12, fontSize:13, color:'#00536A' }}><b>Récap :</b> {missions.filter(m=>selComb.includes(m.id)).reduce((s,m)=>s+(m.passagers||0),0)} pass. · {missions.filter(m=>selComb.includes(m.id)).map(m=>m.destination).join(' + ')}</div>
      ...
      {psForm && (
        <Modal title={psForm.mode==='pays'?`✏️ Renommer "${psForm.pays}"`:`✏️ Renommer "${psForm.oldVal}"`}
          onClose={() => setPsForm(null)}
          onSubmit={async () => {
            if (psForm.mode === 'pays') {
              await supabase.from('pays_sites').update({ pays:psForm.val }).eq('pays', psForm.pays)
              await supabase.from('vehicules').update({ pays:psForm.val }).eq('pays', psForm.pays)
            } else {
              await supabase.from('pays_sites').update({ site:psForm.val }).eq('id', psForm.id)
              await supabase.from('vehicules').update({ site:psForm.val }).eq('site', psForm.oldVal).eq('pays', psForm.pays)
            }
            fetchPaysSites(); setPsForm(null); notify('Renommé avec succès.')
          }}>
